...
export default class ChatHandler {
    ...
    static async deleteSession(req, res) {
        ...
        const session = await ChatSessionSchema.findOne({ _id: sessionId, userId });
        if (!session)
            throw new AppError(StatusCode.ClientErrorNotFound, "Session not found!", "NotFound");
    ...
    static async getHistory(req, res) {
        ...
        // @ts-ignore
        if (date && isNaN(date))
            throw new AppError(StatusCode.ClientErrorBadRequest, "Enter valid date!", "BadRequest");
        const filter = { userId };
        if (date) {
            // @ts-ignore
        ...
        if (sortByDate) {
            ...
            for (const message of historyMessages) {
                if (!message.metadata)
                    continue;
                const dateCollectionIndex = messagesByDate.findIndex(dateCollection => {
